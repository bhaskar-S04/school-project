...
export default async function handler(req, res) {
  ...
  form.parse(req, async (error, fields, files) => {
    try {
      ...
      const nameValue = Array.isArray(fields.name)
        ? fields.name[0]
        : fields.name;
      const addressValue = Array.isArray(fields.address)
        ? fields.address[0]
        : fields.address;
      const cityValue = Array.isArray(fields.city)
        ? fields.city[0]
        : fields.city;
      const stateValue = Array.isArray(fields.state)
        ? fields.state[0]
        : fields.state;
      const contactValue = Array.isArray(fields.contact)
        ? fields.contact[0]
        : fields.contact;
      const emailValue = Array.isArray(fields.email_id)
        ? fields.email_id[0]
        : fields.email_id;
